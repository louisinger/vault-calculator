...
export async function transferLBTCToAccount(
  namespace: string,
  amount: number,
  sum: number
): Promise<string> {
  const marina = await detectProvider('marina');
  if ((await marina.getSelectedAccount()) !== namespace)
  ...
  const tx = await marina.sendTransaction([
    {
      address: (
        await marina.getNextAddress({ artifact, args: { sum } })
      ).confidentialAddress,
      value: amount,
      asset,
    },
...
export function findRedeemLeaf(
  tree: bip341.HashTree
): { leaf: bip341.HashTree; sum: number; xonlypubkey: string } | undefined {
  if (!tree.scriptHex) {
